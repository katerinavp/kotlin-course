package lessons.lesson17.homeworks

import lessons.lesson14.homeworks.Main
//Basa Class нельзя менять
abstract class BaseClass(
    // 1. объясни, почему это поле доступно в main() для чтения из класса ChildrenClass
    //в ChildrenClass объявлено своя переменная  private val privateVal: String
    private val privateVal: String,
    // 2. объясни, почему это поле недоступно в main()
    // protected доступен только из дочерних классов и самого класса
    protected val protectedVal: String,
    val publicVal: String
) {
    var publicField = "3. измени меня из функции main() на Антонио Бандераса и проверь через функцию getAll()" +
            "4. Доработай ChildrenClass таким образом, чтобы это получилось"
        set(value) {
            if (verifyPublicField(value)) {
                field = value
            }
        }
    protected var protectedField = "5. измени меня из функции main() через сеттер в наследнике"
    private var privateField = "6. добавь сеттер чтобы изменить меня из main()"//нельзя выполнить, тк поле private
    fun getAll(): String {
        return mapOf(
            "privateVal" to privateVal,
            "protectedVal" to protectedVal,
            "publicVal" to publicVal,
            "publicField" to publicField,
            "protectedField" to protectedField,
            "privateField" to privateField,
            "generate" to generate(),
        ).map { "${it.key}: ${it.value}" }
            .joinToString("\n")
    }

    fun printText() {
        privatePrint()
    }

    // 7. объясни, почему эта функция не может быть публичной
    //тк класс protected он доступен только в родительском и дочернем классе , функция тоже доступна из родителя и дочерних классов. Извне обратится нельзя
    protected open fun getProtectedClass() = ProtectedClass()
    protected open fun verifyPublicField(value: String): Boolean {
        return value.length < 3
    }

    // 8. Распечатай getAll() и объясни, почему в поле "generate" другой текст
    // поле generate вызывается текст из generate метода дочерних классов (переопределенный метод)
    open fun generate(): String {
        return "Это генерация из родительского класса"
    }

    private fun privatePrint() {
        println("Печать из класса BaseClass")
    }

    // 9. объясни, почему эта функция не может быть публичной или protected
    // приватный метод доступен только внутри класса ,где метод создан
    private fun getPrivateClass() = PrivateClass()

    protected class ProtectedClass() {}

    private class PrivateClass() {}
}

class ChildrenClass(
    val privateVal: String,
    protectedVal: String,
    // 10. объясни, почему этот аргумент доступен в main() несмотря на то, что это не поле
    // аргумент конструктора ChildrenClass, который не является полем
    publicVal: String
) : BaseClass(privateVal, protectedVal, publicVal) {
    // 11. объясни, почему в main() доступна функция getAll() хотя её здесь нет
    //getAll public, и он доступен в дочерних классах и извне
    // 12. проверь, что выводится на печать при вызове функции printText()
    // и объясни, почему не происходит переопределение метода privatePrint()
    //потому что privatePrint приватный в родительском классе и он не доступн в дочернем. Мы не можем приватные сущности переопределить в дочернем

    private fun privatePrint() {

        println("Печать из класса ChildrenClass")
    }

    override fun generate(): String {
        return "Это генерация из дочернего класса"
    }

    override fun verifyPublicField(value: String): Boolean {
        return value.length < 51
    }

   fun setProtectedField(value:String){
         protectedField = value
    }
}


fun main() {
    val childrenClass = ChildrenClass("private", " protected", " public")
    childrenClass.publicField = "publicFieldChange"
    print("getAll ${childrenClass.getAll()}")
    childrenClass.setProtectedField("5. измени меня из функции main() через сеттер в наследнике -  ИЗМЕНИЛА")
    print("printText ${childrenClass.printText()}")
}
